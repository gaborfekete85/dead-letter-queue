package guru.learningjournal.examples.kafka.avroposfanout.util;
...
public class Kafkautil {
    ...
    public static <K, V> KafkaConsumer getConsumer(String autoOffset) {
        KafkaConsumer<K, V> kafkaConsumer = new KafkaConsumer<>(getKafkaProperties(autoOffset));
        return kafkaConsumer;
    }
    public static Properties getKafkaProperties(String autoOffset) {
        Properties kafkaProps = new Properties();
        kafkaProps.put("bootstrap.servers", "localhost:9092");
        kafkaProps.put("application.id", "SpringApp");
    ...
    public static <T> byte[] serializeAvro(ConsumerRecord<?, T> event, Schema schema) {
        try {
    ...
    public static <T> T deserializeAvro(byte[] data, Schema schema, Class<T> cls) throws IOException {
        try {
